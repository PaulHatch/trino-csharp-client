...
/// </summary>
public class Records : IEnumerator<List<object>>,
#if NET8_0_OR_GREATER
        IAsyncEnumerator<List<object>>,
#else
    IAsyncEnumeratorPlaceholder<List<object>>,
#endif
    IDisposable
{
    /// <summary>
    ...
    public async Task<bool> MoveNextAsync()
    {
        ...
        if (pages.IsEmptyPage || IsAtEndOfPage())
        {
            ...
            if (await pages.MoveNextAsync().ConfigureAwait(false) && pages.Current?.HasData == true)
            {
    ...
    private T CastWithNullCheck<T>(int colIndex, object o, bool allowNull)
    {
        if (o == null && !allowNull)
        {
            throw new NullReferenceException($"Value in column {colIndex} of type `{Columns[colIndex].Type}` is null, and requested type `{typeof(T).Name}` is not nullable.");
        }
        try
        {
...
#if NET8_0_OR_GREATER
        ValueTask<bool> IAsyncEnumerator<List<object>>.MoveNextAsync()
        {
