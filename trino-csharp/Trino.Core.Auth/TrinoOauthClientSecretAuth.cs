...
namespace Trino.Core.Auth
{
    public class TrinoOauthClientSecretAuth : ITrinoAuth
    {
        ...
        {
            if (TokenEndpoint is not { Length: > 0 } tokenEndpoint ||
                ClientId is not { Length: > 0 } clientId ||
                ClientSecret is not { Length: > 0 } clientSecret ||
                Scope is not { Length: > 0 } scope)
            {
            ...
            tokenExpiry = DateTime.UtcNow.AddSeconds(tokenResponse?.ExpiresIn ?? 0);
        }
